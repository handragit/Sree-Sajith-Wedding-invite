...
import { CalendarDays, Clipboard, MapPin } from "lucide-react";
...
export default function VenueCard({ event }: { event: WeddingEvent }) {
  ...
  return <article className="venue-card">
    ...
    <div><h3>{event.venue}</h3><address>{event.address}</address><div className="actions"><a className="button button--solid" href={event.mapUrl} target="_blank" rel="noreferrer"><MapPin size={17} /> Get Directions</a>{event.id === "reception" && <CalendarLink event={event} />}<button className="button button--outline" onClick={copyAddress} type="button"><Clipboard size={17} /> {copyStatus === "copied" ? "Address copied" : copyStatus === "error" ? "Copy unavailable" : "Copy Address"}</button></div><span className="sr-only" aria-live="polite">{copyStatus === "copied" ? "Address copied to clipboard." : copyStatus === "error" ? "The address could not be copied. Select the address text to copy it manually." : ""}</span></div>
  </article>;
}
