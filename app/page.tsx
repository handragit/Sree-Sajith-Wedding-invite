...
function RegistryWishlist({ items }: { items: PublicWishlistItem[] }) {
...
export default async function Page() {
  let wishlistItems: PublicWishlistItem[] = [];
  try {
    wishlistItems = await getPublicWishlistItems();
  } catch {
    // The invitation remains usable if public registry data is temporarily unavailable.
  }

  return <>
    <a className="skip-link" href="#main">Skip to content</a>
    <nav className="nav" aria-label="Main navigation"><a className="monogram" href="#home" aria-label="Sajith and Sreelakshmi, home">{wedding.couple.monogram}</a><div className="nav-links">{[["Events", "events"], ["Registry", "registry"], ["RSVP", "rsvp"]].map(([label, id]) => <a key={id} href={`#${id}`}>{label}</a>)}</div></nav>
    <main id="main">
      <header id="home" className="hero"><div className="card-border" aria-hidden="true" /><div className="hero-inner"><GaneshaMark /><h1><span>Sajith</span><em>&amp;</em><span>Sreelakshmi</span></h1><p className="invitation-line">A celebration of love, family,<br />and new beginnings.</p><div className="hero-details"><strong>13–14 December 2026</strong><span>Thrissur, Kerala</span></div></div></header>
      <section className="parents section"><p className="eyebrow">WITH THE BLESSINGS OF OUR PARENTS</p><h2>{wedding.parents[0]}</h2><span className="and">and</span><h2>{wedding.parents[1]}</h2><p>together with the blessings of our grandparents and elders,<br />we invite you to join us in celebrating our wedding.</p></section>
      ...
      <section id="registry" className="registry-section section"><div className="registry-inner"><div className="registry-kolam" aria-hidden="true" /><h2>Registry</h2><p className="registry-lead">Your presence and well wishes are more than enough.</p><p className="registry-copy">Gifts are not expected. If you genuinely wish to give something, this registry will offer a simple way to help reduce duplicate gifting and unnecessary waste, in keeping with the near-zero-waste philosophy of our wedding.</p><div className="registry-rule" aria-hidden="true" /><RegistryWishlist items={wishlistItems} /></div></section>
